...
|---|---|---|
| **Procedure** | stimulus-annotator, temporal-annotator | `@reinforcer`, `@sd`, `@brief`, `@clock`, `@warmup`, `@algorithm` |
...
### 採用例: stimulus-annotator の `@reinforcer` family
...
詳細は [annotations/stimulus-annotator/README.md](../annotations/stimulus-annotator/README.md) §Keyword Aliases を参照。
...
上記 DIVERGENCE A/B/C は次回の annotation 層整備セッションで集中的に扱う。
優先度は C（Measurement 新設）> B（`@hw` 多義性）> A（`@algorithm` 位置）の順。
