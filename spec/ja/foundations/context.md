...
> contingency-dsl 基盤層（Ψ）の一部。文脈を基盤的構成要素として定義し、フェーズ Ψ-10 および `contingency-respondent-dsl` 拡張パッケージで予定されている更新（renewal）、再発（reinstatement）、文脈駆動レスポンデント手続きを支える。
