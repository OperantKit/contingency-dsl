...
representation 変換に関わるメタデータは、以下の 2 層に分離される。

```
┌─────────────────────────────────────────────┐
│  Layer 0: Source Annotations                 │  Functor Law で保存保証
│  @reinforcer("pellet"), @clock(unit="s")     │  全 representation 共通
└─────────────────────────────────────────────┘
┌─────────────────────────────────────────────┐
│  Layer 1: Representation Metadata            │  各 representation が自由に定義
│  TTauApprox.assumption = IRT(2.0)            │  annotation set とは型レベルで分離
│  (将来) RateSpace.fitting_method = ...       │  逆変換で破棄
└─────────────────────────────────────────────┘
```

**Layer 0（Source Annotations）** は DSL ソース式に由来するメタデータであり、
前述の Functor Law により全 representation 変換を越えて保存される。
変換の入出力が共有する不変の情報層である。

**Layer 1（Representation Metadata）** は変換過程で生成される、
特定の representation に固有のメタデータである。
これは annotation set には混入させず、result type の構造的フィールドとして表現する。

#### Representation Metadata の要件

各 representation は、変換過程で生成される固有のメタデータを
独自の typed field として宣言してよい。ただし以下を満たすこと:

| # | 要件 | 根拠 |
|---|------|------|
| M1 | metadata field は representation の result type に閉じる | annotation set との分離を保証する |
| M2 | 逆変換（backward）時に metadata は破棄される | 復元されたスケジュール式に representation 固有情報が残留してはならない |
| M3 | 新 representation 追加時、metadata schema を `representations/<name>/metadata.schema.json` として提出する | 機械的検証を可能にする |
| M4 | metadata field の型は JSON Schema で記述可能でなければならない | 言語非依存性（§1 の第一目標）を維持する |

#### Program-level annotation のスコープ

Program-level annotation（`@species("rat")` 等）はスケジュール単位の
representation 変換の定義域外であり、変換の入出力に含まれない。
プログラム AST においてスケジュールノードより上位の階層に属する annotation は、
スケジュール単位の座標変換に参加せず、元の階層に留まる。

#### 具体例

Source annotation の保存（Layer 0）:
...
近似変換における 2 層の分離（Layer 0 + Layer 1）:
...
出力:   TTauApprox(T=10, tau=10, assumption=IRTAssumption(irt=2.0))
            @reinforcer("pellet")
        ↑ Layer 0: annotation は保存
        ↑ Layer 1: assumption は TTauApprox 型の構造的フィールド
                ↓ from_ttau(domain="R", distribution="F")
復元:   FR 5 @reinforcer("pellet")
        ↑ Layer 0: annotation は保存
        ↑ Layer 1: assumption は破棄（要件 M2）
```

将来の representation 追加時の例:

```
representations/rate-space/
├── README.md
├── metadata.schema.json        ← 要件 M3: fitting_method 等を宣言
└── conformance/
    ├── to_rate_space.json
...
└── t-tau/
    ...
    ├── metadata.schema.json # Representation Metadata の JSON Schema（§5 要件 M3）
...
`metadata.schema.json` は representation 固有メタデータがない場合でも
空スキーマ（`{}`）として提出し、存在を保証すること。
...
| Layer | 内容 |
|-------|------|
| 1. 機械的検証 | 変換規則の形式的定義、conformance テスト JSON の存在、annotation 保存テスト（§5）、metadata.schema.json（§5 要件 M3） |
