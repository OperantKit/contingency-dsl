...
└── context/     — 環境コンテキスト (@context; v2.0, renewal/ABA デザイン用)
```
...
- 2026-04-12: 初版。以下の再編を実施:
  - `stimulus-annotator` + `temporal-annotator` + `context-annotator` → `procedure-annotator/` (stimulus + temporal + context sub-annotators)
...
- **Steady-state criteria の自動判定**: データ解析層の責務。DSL で判定すると「手続きと効果を混同」する。ただし安定性**基準の宣言**（いつ安定とみなすか）は measurement-annotator の `@steady_state` として実装済み（2026-04-12）。ここで「範囲外」とするのは基準に基づく**自動判定の実行**である。
