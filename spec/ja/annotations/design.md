...
- 本来 Measurement に属すべき annotation の例:
  ...
  - `@steady_state(window_sessions=5, max_change_pct=10, measure="rate")` — 定常性基準
...
| **プログラムレベル（Measurement）** | `@session_end`, `@baseline`, `@steady_state` | 測定基準はセッション全体に適用。定常性判断・ベースライン条件は境界条件 |
...
#### 8.1.1 `@session_end` — セッション終了条件 **[実装済み]**
...
- **Steady-state criteria の自動判定**: データ解析層の責務。DSL で判定すると「手続きと効果を混同」する。ただし定常性**基準の宣言**（いつ安定とみなすか）は measurement-annotator の `@steady_state` として実装済み（2026-04-12）。ここで「範囲外」とするのは基準に基づく**自動判定の実行**である。
...
1. ~~`@session_end`~~ — **実装済み** (measurement-annotator v1.x, 2026-04-12)
2. `@response` — 必須化すべき
