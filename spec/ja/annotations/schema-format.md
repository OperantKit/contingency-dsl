...
>
> annotation-design.md §1 および §7.1 で指摘されていた「メタDSL 未定義」
> 問題を解決する。
...
来歴は括弧内に記載可: `"Schema Design (restructured 2026-04-12)"`
...
`positional` が `null` の場合、その keyword はキーワード引数のみを受け付ける。
...
**制約:**
- `required_if` と `forbidden_if` のリストは重複してはならない
...
**制約:**
- `default_expr` は同一 keyword の `params` 内に存在するパラメータ名
...
### 7.0 `array<T>` と `object`（構造化値、v0.1）

RFC 2026-04-17 により additive grammar 拡張（design-philosophy §8.1）として
追加。対応する grammar プロダクション `annotation_array` / `annotation_object`
は [grammar.ebnf §4.7](../../../schema/core/grammar.ebnf) に定義。
...
理論的根拠は annotation-design.md §3.5 を参照。
...
すべての registry が同一スキーマフォーマットを共有するため、
スキーマレベルでの相互運用性が保証される。

### 11.3 Composite registry

プログラムは複数のスキーマファイルから registry を合成してよい。
keyword 名の衝突はエラーとする（同一 annotator 内の alias → primary
マッピングは例外）。
...
grammar.ebnf は annotation の構文形を定義する。本仕様は
**どの `annotation_name` が存在し、どの引数形を受け付けるか**を定義する。
...
2026-04-13 時点で以下のスキーマファイルが本仕様に適合する:
