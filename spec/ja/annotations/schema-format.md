...
型語彙は意図的に最小限にしてある（DSL grammar の `annotation_val` と
一致）。将来 annotation が構造化値やネスト値を必要とする場合は、
grammar の `annotation_val` への additive 拡張（design-philosophy §8.1）
と本仕様の型識別子追加を同時に行う。

現在の `annotation_val` がサポート**しない**もの:
- 配列 / リスト
- ネストオブジェクト
- スケジュール式への参照
- ブール値
