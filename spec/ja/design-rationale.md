...
> **ステータス:** 参考情報
>
> 本ドキュメントは、注釈アーキテクチャおよび層構造について検討した
> 代替案を記録し、現行設計が選ばれた理由を説明するものである。
...
六層アーキテクチャ (Foundations / Operant / Respondent / Composed /
Experiment / Annotation) は [design-philosophy.md §2](design-philosophy.md)
に定義されている。本節は、検討された他の 2 つのアーキテクチャを差し置いて
六層構造が選ばれた理由を記録する。

### 5.1 検討した代替案

**Case A (却下)** — トップレベルディレクトリに抽象軸を採用:
...
**Case B (却下)** — レスポンデントのプリミティブを注釈のみで表現:
...
- **短所:** CS-US 随伴性の構造 (Pavlov, 1927; Rescorla, 1967, 1968) は
  **文法レベル**の事象であり、メタデータレベルの事象ではない。
  ...
  Case B はこのテストに失敗する。評価に影響しないことを契約で保証する
  層に、文法的に load-bearing な構築物を置こうとするためである。

**Case C（採用）** — レスポンデントの深さに対する拡張ポイントを備えた
