...
> contingency-dsl 実験層（Ψ）の一部。`PhaseSequence` 構成要素、`Phase` 要素とその注釈継承規則、`no_schedule` フェーズ変種、および設計を動機づける JEAB 準拠の Method セクション慣習を定義する。本ファイルは人間向けの仕様である。対応する JSON Schema は `schema/experiment/phase-sequence.schema.json` にある。
