...
**デフォルト動作:** 対称（切替方向によらず同一の遅延）、リセット型（各切替で新しい COD タイマーが開始される）。これは標準的定義——強化は「最後の切替から」一定時間経過後にのみ利用可能（Catania, 1966）——に従う。
...
**将来:** リセット型 COD（COD 作動中の再切替でタイマーがリスタートする）。
...
**将来:** 非対称 BO（遷移方向ごとに異なる値。例: Rich→Lean vs. Lean→Rich）。非対称 COD（Issue #25）と同期して設計。
...
| **典型的文脈** | 実験的研究 | トークンエコノミー、ヒトオペラント |
| **四象限** | 負の罰（SP−） | 負の罰（SP−） |

TO とレスポンスコストはどちらも負の罰の事例であるが、作用対象が異なる：TO は「未来」に作用し（まだ得られていない強化が一定時間利用不可能になる）、レスポンスコストは「過去」に作用する（既に付与された条件性強化子が回収される）。Leitenberg (1965) が操作的区別を最初に定め、Kazdin (1972) がレスポンスコスト文献を包括的に統合した。

**宣言のみの契約。** DSL は随伴性「応答 → unit を amount だけ除去」を宣言する。以下は宣言しない:

- 初期のトークン付与量（セッション/プロトコル層の責務）。
- 残高ゼロ時の挙動（破産、応答ブロッキング、セッション終了——すべて記録層のポリシー）。
- トークン残高のセッション間持ち越し。
- トークンの一次強化子への換算率（`@reinforcer` アノテーションまたは token-economy 拡張の責務）。

これは DSL の状態に対する広範な立場と整合する：セッション内の状態機械はスケジュールの denotation（§2.13）に内在するが、縦断的な状態蓄積（残高、履歴、破産）は記録/実行層に委ねられる。

**トークン強化との関係。** レスポンスコストは条件性強化子に対して作用する——除去されるべき「何か」が存在する前提である。条件性強化子の宣言（`@reinforcer` アノテーション）は推奨されるが必須ではない。強化子を宣言せず `ResponseCost(...)` を使用するプログラムはリンター警告（`RC_WITHOUT_TOKEN_CONTEXT`; grammar.ebnf 制約 §96）を発し、除去対象のトークンが未規定であることを示す。ただしこの警告は一次強化子と条件性強化子を区別しない——`@reinforcer("food")` を宣言すれば food がトークンでなくても警告は抑制される。この区別を精緻化するには reinforcer-type annotation の拡張が必要であり、core DSL の範囲外である。Hackenberg (2009) が現代のトークン強化文献を統合し、そこでは "token" が優勢な用語となっている。これに基づき `unit="token"` をデフォルトとする。`unit="point"` は Weiner (1962, 1963) への忠実さのために保持される。

**経験的根拠。** Weiner (1962) は、レスポンスコストがスケジュール種別（VI と FI）にまたがって差異的な抑制を産出すること、すなわち随伴性構造（応答あたりの減算）がスケジュール構造（インターバル配置）と直交することを示した。Weiner (1963) はさらに、ポイント損失随伴性がショックの不在下でもヒトの回避・逃避反応を維持できることを示し、レスポンスコストを一般的な嫌悪制御手続きとして確立した。Kazdin (1972) は臨床文献をレビューし、トークンエコノミーにおける行動抑制の支配的機序としてレスポンスコストを位置づけた。Hackenberg (2009) は現代のトークン強化文献を統合した。
...
ここで `v⃗ᵢ = (v₁ᵢ, …, vₘᵢ)` は zip された steps 変数の i 番目の列。

**ステップ 2 — 挿入位置 i ごとの gap block を構築。**

```
B(i) = [ clone(R(r₁), Name, i), clone(R(r₂), Name, i), …, clone(R(rₖ), Name, i) ]
```

`R(rⱼ)` はラベル `rⱼ` で事前宣言された template phase を解決し、`clone(P, Name, i)` は `P` の deep copy を返し、`label` を `P.label ++ "_after_" ++ Name ++ "_" ++ str(i)` に書き換える。他のフィールド（`schedule`、`criterion`、`phase_annotations`、`phase_param_decls`）は verbatim にコピーされる。

**ステップ 3 — Intercalate（既定）または intersperse（`no_trailing` 指定時）。**
...
**定義 17（実験マシン）。** 実験マシンはフェーズ列をスケジュールマシンの列に写像する:
