...
**非対称 COD（asymmetric COD）。** 切替方向によって切替コストが異なる場合、COD は各コンポーネントからの離脱時の遅延を値リストで受け付ける（Pliskoff, 1971）。要素 *i* はコンポーネント *i* を**離れる**ときに課される COD を指定する:

```
Conc(VI 30-s, VI 60-s, COD=[2-s, 5-s])    -- VI30 から離脱時 2s、VI60 から離脱時 5s
Conc(VI 30-s, VR 20, COD=[1-s, 3-s])      -- 並立 interval-ratio での非対称コスト
                                           -- （Sakagami, 1989）
```

非対称 COD の意味論的規則:
- 値リストの長さは位置引数（コンポーネント数）と**一致しなければならない**。不一致 → `ASYMMETRIC_COD_LENGTH_MISMATCH`。
- 全要素が同一値の場合、リンター WARNING `REDUNDANT_ASYMMETRIC_COD` を発行。スカラー形式の使用を推奨。
- プログラムレベル `param_decl` はスカラー COD のみ受け付ける（対称デフォルト）。式レベルの非対称 COD はこれを完全に上書きする。
- 各要素の検証: スカラー COD と同じ規則に従う（時間単位必須、≥ 0）。

**per-departure-component 意味論。** COD は被験体が**離れる**コンポーネントに紐づく。特定の遷移ペア（出発–到着）には紐づかない。これは Pliskoff (1971) 以降の非対称 COD 研究における標準的な実験操作を再現する。COD が特定の出発–到着ペアに依存するまれなケース（n×(n−1) 行列）については `contingency-core` 層を参照。
