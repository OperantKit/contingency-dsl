...
CER は実験層でフェーズの並びとして合成される: オペラント・ベースラインで反応率を確立し、続くペアリング・フェーズで同じベースライン上にパヴロフ型 CS–US 随伴性を重畳し、テスト・フェーズで（典型的には CS を導入した状態の）ベースラインに復帰して抑制比を測定する。注釈（`../annotations/extensions/respondent-annotator.md` の `@cs`, `@us`）は刺激メタデータを記録する。
...
各フェーズは単一のスケジュール本体を持つ。ペアリング・フェーズはパヴロフ型 primitive をオペラント・ベースラインに代えて配置する。US はオペラント反応クラスから独立にパヴロフ型スケジュールで呈示される。この時間的独立性が、CER をオペラント・ベースラインへの罰手続きではなく合成手続きたらしめる。テスト・フェーズの `use baseline` は、ベースラインのスケジュールを参照によって再導入する形式である。
...
CER は次の経験的パターンの標準的デモンストレーションである: パヴロフ型（反応独立）配置で嫌悪的 US と対提示された刺激を、後続して食欲性オペラント・ベースライン中に呈示すると、ベースライン反応率が抑制される。この抑制は、US がオペラント反応に随伴して配送されたことがない条件下で得られる。

Rescorla and Solomon (1967) はこのパターンに対して一つの理論的説明を提供する: 2 過程枠組み（[two-process-theory.md](two-process-theory.md) 参照）の下では、CS–shock 配置が条件性嫌悪的「中枢状態」を確立するとされ、食欲的状態と嫌悪的状態は同枠組みの中で動機づけ的に非両立として扱われるため、その状態が食欲性オペラント反応を抑制するとされる。「中枢状態」「条件性嫌悪状態」「動機づけ的非両立性」といった語彙は Rescorla and Solomon の語彙であり、DSL は上段で述べた経験的パターンを記録するのみで、2 過程理論がその解釈を一つ提供するものだと注記する。
