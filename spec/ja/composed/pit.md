...
3. **転移テスト.** 道具的反応が利用可能な状態で、パヴロフ型 CS が呈示される。典型的には、CS 呈示中の反応率変化がテスト内の反応随伴強化に帰属されえないことを保証するため、消去下（テスト中に US 呈示なし）で行う。
...
- **一般 PIT.** パヴロフ型訓練と道具的訓練は **同じ食欲性または嫌悪性クラス** の結果を用いるが、必ずしも同一の結果ではない（例: パヴロフ型 CS をスクロースと対提示; 道具的反応を餌ペレットで強化）。これらの訓練条件の下では、その後の CS 呈示が道具的反応率を増加させる。パヴロフ型訓練で用いられた特定の結果が道具的反応に随伴して配送されたことはない。2 過程理論（[two-process-theory.md](two-process-theory.md) 参照）は、このパターンに対して共有された動機づけ状態の語彙での解釈を一つ提供するが、DSL はその語彙を自身の声で採用せず、手続き的配置と経験的パターンを記録するに留める。
- **結果特異 PIT.** パヴロフ型 CS と道具的反応は **同一の結果** を共有する（例: 両者ともスクロースで訓練）。複数の結果特異反応を同時にテストする場合、CS はその特定結果に対する道具的反応を選択的に増加させる。この変種は文献上、パヴロフ型段階における結果特異な S–O 連合の証拠とされる。
...
各フェーズは単一のスケジュール本体を持ち、別セッション（またはセッション内で明確に分離されたブロック）で作動する。フェーズの順序がこの時間的分離を保証する。転移テストは `use instrumental_training` でオペラント・ベースラインを参照によって再導入する。テスト時にはパヴロフ型 CS を提示し（典型的にはプローブ試行として、その呈示スケジュールは `@cs_interval` 注釈で記述）、道具的反応への CS の調整効果を定量化する。
