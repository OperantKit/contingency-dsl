...
```
Conc(VI30s, VI60s, FRCO=5)            -- 切替後 5 反応が必要
Conc(VI30s, VI60s, COD=2s, FRCO=5)   -- 時間と反応の両方の要件
```
...
COD と FRCO はいずれも `param_decl` によるプログラムレベルのデフォルトをサポートする（`LH = 10s` と同様）:
...
```
Sidman(SSI=20s, RSI=5s)                              -- 基本形
SidmanAvoidance(SSI=20s, RSI=5s)                     -- verbose alias
Sidman(ShockShockInterval=20s, ResponseShockInterval=5s)  -- verbose parameter
```
...
これは第 1 link で food の FR10 を完了し、第 2 link で自由オペラント回避に
...
差異強化修飾子 DRL、DRH、DRO はいずれも **時間次元**（反応間間隔）に対して
動作する。これらとは異なる種類の差異強化として、**反応の変動性（variability）
...
本 DSL は `Lag` を差異強化修飾子として `modifier` 生成規則（grammar.ebnf）に
...
- `n` に時間単位を付けることは不可。`Lag 5s` → `LAG_UNEXPECTED_TIME_UNIT`。
  ...
  文献上存在しない（時間ベースの差異強化は DRL/DRH、嫌悪系の時間パラメータは
...
- Lag スケジュールに時間次元の variant は確立していない。時間ベースの
  variability 研究は別手続き（Platt, 1973 の反応持続時間差異強化、
