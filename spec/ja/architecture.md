...
6層の責務分離を提案する。計算能力の次元、反応機会の構造の次元、および実験デザインの構造の次元で関心を分離する:
...
│   │ 自由オペラント      │ │  パラメータ宣言的、基準はランタイム算出
│   │ criterion =       │ │  Pctl(IRT, 50), Adj(start=FR1, step=2)
│   │   f(runtime_state)│ │
│   ├───────────────────┤ │
│   │ Core-TrialBased   │ │  CFG 構文、離散試行
│   │ 離散試行           │ │  刺激–反応マッチング基準
...
| **contingency-dsl (Core-TrialBased)** | CFG 構文、離散試行 | 離散試行手続き。刺激–反応マッチング基準 | `MTS(comparisons=3, consequence=CRF, ITI=5s)` |
...
| 性質 | Core | Core-Stateful | Core-TrialBased | contingency-core |
|---|---|---|---|---|
| AST 構造 | パース時固定 | パース時固定 | パース時固定 | セッション中に変化 |
| パラメータ | リテラル値 | リテラル値 | リテラル値 | 計算値・切替可能 |
| 基準値 | リテラルとの比較 | f(runtime_state)、ルールはパース時固定 | 刺激–反応マッチング | 任意の状態に依存可能 |
| 反応機会 | 自由オペラント（連続） | 自由オペラント（連続） | 離散試行 | 任意 |
| 結果事象 | 暗黙的 | 暗黙的 | 明示的（Core schedule 参照） | 任意 |
| スケジュール同一性 | 不変 | 不変 | 不変 | 別スケジュールへ遷移 |
| 計算モデル | CFG（非TC） | CFG 構文, TC 近傍評価 | CFG 構文 | チューリング完全 |
