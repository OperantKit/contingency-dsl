...
2. いずれの追加も **additive only**（既存の構文・意味を変更しない）と
   する。additive な変更は `versioning.md` に設計チェックポイントとして
   記録する。
...
1. `versioning.md` に **設計チェックポイント** として変更を記録し、
   §8.2 のいずれの条件に該当するか、および旧構造・新構造を明示する。
   当リポジトリは現時点で外部公開チェックポイントを持たない（git remote
   未設定）; 設計チェックポイントが正式な記録である。
2. 変換ツールを同時に提供し、直前の設計チェックポイントで書かれた DSL を
   新しいチェックポイントに機械的に移行可能にする。
