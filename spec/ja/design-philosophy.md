...
この目標を達成するため、**Core 文法の安定性を最重要視し、破壊的変更を
必要としない設計を追求する**（詳細は §2 および §8 を参照）。
...
Core と Core-Stateful はいずれも**全プログラム共通**の共有語彙である。
構造的な区別: Core のスケジュールは基準がリテラル値（`FR 5`: 基準 = 5反応）、
Core-Stateful のスケジュールは基準がランタイム状態から算出される
（`Pctl(IRT, 50)`: 基準 = 直近 IRT 分布の中央値）。詳細は §2.1 を参照。
...
Core-Stateful は、**学派で確立された**が、強化基準の評価にランタイム状態を
必要とするスケジュールを収容する。Core と同じく全プログラム共通の共有語彙で
あるが、評価に関する静的決定可能性は Core の完全な決定可能性とは異なる。

**構造的区別:**

| 性質 | Core | Core-Stateful |
|---|---|---|
| 構文 | CFG | CFG |
...
**Admission gate:** スケジュールが Core-Stateful に昇格するためには、
(1) 以下の学派確立基準を充足し、
...
**昇格プロセス:** スケジュールを Core-Stateful に昇格させる際は、以下の手順に従う。
...
3. **実装:** 承認後、`schema/core-stateful/grammar.ebnf` への生成規則追加、
   ast-schema.json への AST ノード追加、Python / Rust 両実装のパーサ更新を
   §8.1 の additive 手順に従って行う。
4. **記録:** 昇格を本節の構成素テーブルに追記し、§5.6 の候補リストから除去する。

**現在の Core-Stateful 構成素:**
...
|---|---|---|---|
| **Procedure** | `procedure-annotator` (stimulus + temporal sub-annotators) | 手続きの記述 | (1) 暗黙的な手続きの明文化, (2) 二つの手続きの等価性判定の支援, (3) DSL と実験プログラムの相互変換に必要な条件の補足 |
| **Subjects** | `subjects-annotator` | 被験体 | 被験体履歴・確立操作・絶食水準・種・状態の記述。現在の随伴性が同一でも行動が異なる要因を記録する。 |
| **Apparatus** | `apparatus-annotator` | 装置・物理実装 | Chamber・operandum・HW 仕様・タイミング許容誤差・物理量の束縛等、Core DSL の抽象を物理世界に接地させる情報。 |
...
## 5. Schedule Extension 層 — 未標準化・ユーザー定義の schedule 構成素

### 5.1 Annotation および Core-Stateful とは別次元の拡張機構

Annotation 層は **metadata**（手続きに付加する情報）を扱う層である。
Core-Stateful 層（§2.1）は、学派で確立されパラメータが宣言的なスケジュールを
共有語彙として収容する。しかし実験手続きには、これらの層に収まらない
**schedule 構造そのものの拡張** が必要なものも存在する。代表例は次の通り:

- **Conjugate reinforcement** — 反応量に比例して強化子が連続的に提示される。
  反応と強化の関係が離散的な schedule event ではない。
  §2.1 の学派確立基準 E2（研究室横断再現）が未充足。
...
§2.1 の学派確立基準を充足していないか、パラメータが計算式で指定されるため、
Core / Core-Stateful の共有語彙にも含められない。

この「Core / Core-Stateful に含められない、かつ annotation でもない」構成素を
収容するため、本 DSL は **Schedule Extension 層** を定義する。

> **歴史的注記:** Percentile schedules（Platt, 1973; Galbicka, 1994）と
> Adjusting schedules は、当初 Schedule Extension の候補として位置付けられて
> いたが、§2.1 の学派確立基準を充足したため Core-Stateful に昇格した。
> この経緯は [rfc-core-constraint-redefinition.md](rfc-core-constraint-redefinition.md)
> および [RFC_CORE_STATEFUL_2026-04-13.md](RFC_CORE_STATEFUL_2026-04-13.md) を参照。

### 5.2 Schedule Extension の位置付け

| 層 | 役割 | closure | 例 |
|---|---|---|---|
| Core | 不変の schedule 基盤（リテラル基準） | 全プログラム共通 | `FR 5`, `Conc(VI 30-s, VI 60-s)`, `Chain(FR 5, FI 30-s)` |
| Core-Stateful | 確立された状態依存基準のスケジュール | 全プログラム共通 | `Pctl(IRT, 50)`, `Adj(start=FR 1, step=2)` |
| **Schedule Extension** | 未標準化・ユーザー定義の schedule 構成素 | program-scoped | 研究室独自スケジュール、conjugate reinforcement |
...
| §2.1 の学派確立基準を充足しているか | （該当しない — metadata は昇格対象外） | No（充足すれば Core-Stateful 昇格候補） |
| Core 既存の schedule 式の評価結果を変えるか | No | Yes（新しい評価を導入する） |
| Source の構文位置 | 式に後置（`FR 5 @name(...)`） | 式そのものの形（`Name(...)`） |

Conjugate reinforcement は反応量と強化子の連続的関係を定義するものであり、
**schedule 式の評価結果を変える**ため annotation では扱えない。学派確立基準が
未充足のため Core-Stateful にも該当せず、Schedule Extension 層に属する。
...
本 DSL プロジェクトは当面 Schedule Extension を**提供しない**。
Percentile および Adjusting は §2.1 の学派確立基準充足により Core-Stateful に
昇格した。残る候補:
