...
```
FR5                          -- 議論のためだけなら十分
Conc(VI30, VI60, COD=2s)     -- 選択手続きの学習
```
...
**4つの保証:**
1. **文法レベルの強制は最小限**: `FR5` 単独で常に parse 成功
...
| `@hw("virtual")` | `"virtual"` | default バックエンド |
...
**`@hw` が virtual 以外（physical HW）のとき必須となる要素。**
**シミュレーション（`@hw("virtual")`）では不要。**
...
| `@hw("teensy41")` 等の具体 HW | simulation から physical への切替 |
...
**依存条件**: `@hw` の値により発動/非発動が決まる（Tier は動的）
...
```python
parse("FR5")                 # OK
parse("Conc(VI30, VI60)")    # OK (COD 省略は semantic warning のみ)
...
|---|---|---|
| 理論議論 | `FR5` | parse |
| 学生の学習 | `Conc(VI30, VI60, COD=2s)` | parse, dev |
| シミュレーション | 上記 + `@clock(unit="s")` + `@algorithm(...)` | parse, dev |
| 実機実験準備 | 上記 + `@hw("teensy41")` + `@session_end(...)` + `@response(...)` + `@operandum(...)` + `@logging(...)` | parse, dev, production |
...
境界テスト (annotation-design.md §2)
  Q1-Q2: Core 文法へ昇格すべきか
    YES → grammar.ebnf を改訂 (design-philosophy §7 の制約下でのみ可)
...
| `@hw`, `@clock`, `@random` | DSL (Tier 1) | 実行パラメータ |
...
# Tier 0-2: @hw の値により Tier 2 要求が発動
...
`@hw` が `"virtual"` なら Tier 2 要素は緩和される:
...
`@hw` の値により Tier 2 発動/非発動が変わる機構は単純だが、
他にも動的 tier が必要か検討する:
