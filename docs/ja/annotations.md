...
- **TD（Sutton & Barto, 1998）**. 時間差分学習: 各時間ステップを状態として扱い、連続する予測の差分から学習する。強化予測誤差（reinforcement-prediction error）の行動的記述と神経科学的記述を橋渡しするモデル。（Sutton and Barto 自身の本文は予測対象を「reward」という技術用語で呼ぶ。本要約は DSL の語彙方針に合わせて「reinforcement（強化）」に置換し、原典と照合する読者のために置換した旨をここで明示する。）
